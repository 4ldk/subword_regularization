...
from utils.utils import f1_score, get_dataloader, path_to_data, dataset_encode
...
class trainer:
    ...
    def forward(self, input, mask, label=None):
        logits = self.model(input_ids=input, attention_mask=mask).logits
    ...
    def train(self, tokenizer, train_dataset, train_loader, num_epoch, valid_loader=None, test_loader=None, p=0):
        ...
        with open("./train_valid_score.csv", "w") as f:
            ...
            max_f1 = 0
            best_epoch = 0
            for i, (loss, f1, tf1) in enumerate(zip(losses, f1s, tes_f1s)):
                f.write(f"{i}, {loss}, {f1}, {tf1}\n")
                if f1 > max_f1:
                    max_f1 = f1
                    best_epoch = i
                elif f1 == max_f1:
    ...
    def step(self, epoch, loader, train=True):
        ...
        for batch_idx, (input, mask, label) in enumerate(bar):
            input, mask, label = (
